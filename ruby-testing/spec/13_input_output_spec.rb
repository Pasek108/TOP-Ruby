...
describe NumberGame do
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      games = [
        described_class.new(5, '5', 2),
        described_class.new(5, '5', 3)
      ]

      # remove the 'x' before running this test
      games.each do |game|
        it "outputs correct phrase for #{game.count}" do
    ...
    context 'when count is 4 and over' do
      games = [
        described_class.new(5, '5', 4),
        described_class.new(5, '5', 5),
        described_class.new(5, '5', 999)
      ]

      # remove the 'x' before running this test
      games.each do |game|
        it "outputs correct phrase for #{game.count}" do
          congrats_phrase = "That was hard. It took you #{game.count} guesses!\n"
          expect { game.final_message }.to output(congrats_phrase).to_stdout
        end
      end
    end
